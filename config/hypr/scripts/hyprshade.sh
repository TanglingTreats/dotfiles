...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ] ;then
        echo "hyprshade_filter=\"$choice\"" > ~/.config/desktop-config/settings/hyprshade.sh
        if [ "$choice" == "off" ] ;then
            hyprshade off
    ...
    if [ -f ~/.config/desktop-config/settings/hyprshade.sh ] ;then
        source ~/.config/desktop-config/settings/hyprshade.sh
    fi
